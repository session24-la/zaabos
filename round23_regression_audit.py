...
checks={
 ...
 'refund_pg_lock':"def refund_order(oid):" in app and "lock_suffix=_for_update(conn)" in app[app.index('def refund_order(oid):'):app.index("@app.post('/api/orders/<int:source_id>/merge')")],
 'split_pg_lock':"lock_suffix=_for_update(conn)" in app[app.index('def split_order(source_id):'):app.index("@app.get('/api/kitchen/stations')")],
 ...
 'quota_pg_lock':app.count("_for_update(conn),(g.tenant_id,)")>=2,
